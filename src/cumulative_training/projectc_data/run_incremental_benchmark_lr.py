...
def make_batches(pool: pd.DataFrame, n: int, seed: int) -> list[pd.DataFrame]:
    """Split pool into n stratified batches preserving open/closed ratio in each."""
    ...
    def _split(rows: pd.DataFrame) -> list[pd.DataFrame]:
        ...
        return [
            rows.iloc[i * size: min((i + 1) * size, len(rows))].reset_index(drop=True)
            for i in range(n)
        ]
    ...
    return [
        pd.concat([o, c], ignore_index=True).sample(frac=1, random_state=seed + i).reset_index(drop=True)
