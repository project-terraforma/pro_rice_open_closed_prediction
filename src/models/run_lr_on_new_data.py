"""
Run Two-Stage LR (No Confidence) model on samples_3k_project_c_updated.csv

This script:
1. Loads the CSV file
2. Parses JSON string fields
3. Renames 'label' to 'open'
4. Runs the Two-Stage LR (No Confidence) model
5. Reports results
"""
...
def parse_json_field(field):
    ...
    try:
        ...
        # Return None for empty arrays/dicts
...
def load_and_prepare_data(csv_path):
    ...
    # Rename 'label' to 'open' (1.0 -> 1, 0.0 -> 0)
    df['open'] = df['label'].astype(int)
    
    # Parse JSON string fields
    json_fields = ['sources', 'names', 'categories', 'websites', 'socials', 
...
def split_data(df, train_ratio=0.7, val_ratio=0.15, random_state=42):
    ...
    train_df, temp_df = train_test_split(
        df, train_size=train_ratio, random_state=random_state, stratify=df['open']
    )
...
if __name__ == "__main__":
    # Load data
    csv_path = os.path.join(os.path.dirname(__file__), '../../data/samples_3k_project_c_updated.csv')
