"""
Unified XGBoost Model for Open/Closed Prediction
...
Usage:
    python xgboost_model.py --mode single --confidence none
    python xgboost_model.py --mode single --confidence source
    python xgboost_model.py --mode two-stage --confidence none
    python xgboost_model.py --mode two-stage --confidence source

Obviously open (Stage 1 filter):
...
class UnifiedXGBoost:
    """
    Unified XGBoost model supporting single-stage and two-stage modes,
    ...
    def _get_variant_name(self) -> str:
        ...
        return f"{mode_str} XGBoost ({conf_str})"
    ...
    def _fit_two_stage(self, train_df: pd.DataFrame, val_df: pd.DataFrame = None):
        ...
        if len(uncertain_train) > 0:
            ...
            # Calculate scale_pos_weight for imbalanced data
            scale_pos_weight = (y_uncertain == 0).sum() / (y_uncertain == 1).sum()
    ...
    def get_feature_importances(self) -> pd.DataFrame:
        ...
        feature_importance_dict = self.model.get_booster().get_score(importance_type='weight')
        
        importances = pd.DataFrame({
            'feature': self._feature_names,
            'importance': [feature_importance_dict.get(f, 0) for f in self._feature_names]
        })
...
def run_all_variants(train_df, val_df, test_df):
    ...
    variants = [
        ("single", False, "Single-Stage XGBoost (No Confidence)"),
        ("single", True, "Single-Stage XGBoost (Source Confidence)"),
        ("two-stage", False, "Two-Stage XGBoost (No Confidence)"),
        ("two-stage", True, "Two-Stage XGBoost (Source Confidence)"),
    ]
    
    results = []
    
    for mode, use_conf, name in variants:
        model = UnifiedXGBoost(mode=mode, use_source_confidence=use_conf)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Unified XGBoost Model")
    ...
    if args.compare_all:
        ...
        model = UnifiedXGBoost(mode=args.mode, use_source_confidence=use_source_conf)
        ...
        model_filename = f"xgb_{args.mode.replace('-', '_')}_{conf_str}.pkl"
