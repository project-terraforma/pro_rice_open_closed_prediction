...
def _sample_params(rng: np.random.Generator, model_key: str) -> dict:
    if model_key == "lr":
        ...
        # - class_weight explores imbalance sensitivity from neutral to closed-heavy.
        cw_choice = rng.choice(["none", "balanced", "custom"])
        if cw_choice == "none":
            class_weight = None
        elif cw_choice == "balanced":
            class_weight = "balanced"
        else:
            closed_w = float(rng.choice([1.5, 2.0, 3.0, 4.0, 5.0]))
            class_weight = {0: closed_w, 1: 1.0}
        return {
    ...
    if model_key == "lightgbm":
        ...
        # - class imbalance knob via either class_weight mode or scale_pos_weight.
        imbalance_mode = rng.choice(["none", "balanced", "scale_pos_weight"])
        if imbalance_mode == "none":
            ...
            class_weight = None
            scale_pos_weight = _sample_log_uniform(rng, 1.0, 20.0)
        return {
    ...
    if model_key == "rf":
        ...
        # - class_weight explores neutral, balanced, and closed-upweighted settings.
    ...
    if model_key == "xgboost":
        ...
        # - scale_pos_weight is the main imbalance knob.
        return {
...
def _normalize_model_params(model_key: str, model_params: dict) -> dict:
    """Normalize deserialized params (e.g., JSON-loaded dict keys) for estimators."""
    params = dict(model_params or {})
    if model_key in {"lr", "rf"}:
        cw = params.get("class_weight")
        if isinstance(cw, dict):
            normalized_cw = {}
            for k, v in cw.items():
                if isinstance(k, str) and k.lstrip("-").isdigit():
                    normalized_cw[int(k)] = v
                else:
                    normalized_cw[k] = v
            params["class_weight"] = normalized_cw
    return params


def _build_model(model_key: str, mode: str, feature_bundle: str, model_params: dict):
    model_params = _normalize_model_params(model_key, model_params)
    if model_key == "lr":
...
def _save_outputs(
    ...
    selected_path = output_dir / "hpo_selected_trials.csv"
    confirm_path = output_dir / "hpo_confirm_metrics.csv"
    config_path = output_dir / "hpo_run_config.json"
