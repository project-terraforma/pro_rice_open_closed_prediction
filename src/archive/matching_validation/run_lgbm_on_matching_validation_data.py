...
It runs a Two-Stage LightGBM flow on `samples_3k_project_c_updated.csv`, where:
...
from lightgbm_model import LightGBMModel, evaluate_model
...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "../../data/archive/matching_validation/samples_3k_project_c_updated.csv")
    df = load_and_prepare_data(csv_path)

    train_df, val_df, test_df = split_data(df, train_ratio=0.7, val_ratio=0.15, random_state=42)

    print("\n" + "=" * 60)
    print("TRAINING TWO-STAGE LIGHTGBM (NO CONFIDENCE) MODEL")
    print("=" * 60)

    model = LightGBMModel(mode="two-stage", use_source_confidence=False, use_interactions=True)
    ...
    results = evaluate_model(test_df["open"], test_predictions, "Two-Stage LightGBM (No Confidence)")
