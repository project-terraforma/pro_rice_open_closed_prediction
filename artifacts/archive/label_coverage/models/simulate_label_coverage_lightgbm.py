#!/usr/bin/env python3
"""
Label Coverage Simulation for LightGBM Model

This script runs the iterative active-labeling simulation using LightGBM
with balanced class weights to improve closed-place prediction.
"""
...
class LabelCoverageSimulation:
    """Main simulation orchestrator for LightGBM model."""
    ...
    def run_batch(
        ...
        if not LIGHTGBM_AVAILABLE:
            print("LightGBM not available, using random predictions.")
            return self._run_dummy_batch(batch_id, batch_df)
        ...
        if len(training_data) == 0:
            ...
            # Merge training data with full records
            train_records = all_data[all_data['id'].isin(training_data['id'])].copy()
            train_records = train_records.merge(training_data[['id', 'label', 'weight']], on='id')
            
            # Convert label column to 'open' format
            train_records['open'] = train_records['label']
            
            print(f"\nTraining on {len(train_records)} records")
            open_pct = (train_records['open'] == 1).mean() * 100
            print(f"  Open: {(train_records['open'] == 1).sum():.0f} ({open_pct:.1f}%)")
            print(f"  Closed: {(train_records['open'] == 0).sum()} ({100-open_pct:.1f}%)")
            
            print(f"\n{'='*60}")
            print(f"Training: LightGBM (Policy)")
            print(f"{'='*60}")
            
            try:
                model = LightGBMModel(mode="two-stage", feature_bundle="low_plus_medium")
                
                # LightGBM uses balanced class weights internally
        ...
        if model is None:
            print("Using random predictions.")
            p_closed = np.random.uniform(0.4, 0.6, len(batch_unlabeled))
        else:
            try:
                proba = model.predict_proba(batch_unlabeled)
                # proba is 2D: [:, 0] is P(closed), [:, 1] is P(open)
                if proba.ndim == 2:
                    p_closed = proba[:, 0]
                else:
                    # Fallback: assume it's P(open) and convert
                    p_closed = 1 - proba
            except Exception as e:
                print(f"Error predicting with LightGBM: {e}, using random predictions.")
                p_closed = np.random.uniform(0.4, 0.6, len(batch_unlabeled))
    ...
    def _run_dummy_batch(self, batch_id: int, batch_df: pd.DataFrame) -> dict:
        """Run a dummy batch when LightGBM is not available."""
        return {
            'batch_id': batch_id,
            'labeled_cumulative': 0,
            'labeled_this_round': 0,
            'n_auto_accept': 0,
            'n_review': 0,
            'n_defer': 0,
            'auto_label_precision': None,
            'auto_conf_mean': None,
            'auto_conf_median': None,
            'auto_conf_10pct': None,
            'auto_conf_90pct': None,
            'test_closed_pr_auc': None,
            'test_closed_f1': None,
            'test_closed_precision': None,
            'test_closed_recall': None,
        }
    
    def _evaluate_on_test(self, model, batch_df) -> dict:
        """Evaluate model performance on frozen test set."""
        try:
            p_open_raw = model.predict_proba(self.test_df)
            # Handle both 1D and 2D array outputs  
            if len(p_open_raw.shape) == 2 and p_open_raw.shape[1] == 2:
                p_open = p_open_raw[:, 1]  # Take probability of positive class (open)
            elif len(p_open_raw.shape) == 2 and p_open_raw.shape[1] == 1:
                p_open = p_open_raw.flatten()  # Single output column
            else:
                p_open = p_open_raw  # Already 1D
...
def main():
    """Run the LightGBM label coverage simulation."""
    print(f"\n{'='*70}")
    print(f"LIGHTGBM LABEL-COVERAGE SIMULATION")
    print(f"{'='*70}")
    
    if not LIGHTGBM_AVAILABLE:
        print("LightGBM is not available. Simulation will use dummy results.")
        ...
        dummy_history.to_csv(output_dir / "simulation_history_lightgbm.csv", index=False)
        print(f"Dummy results saved to {output_dir}")
        return
    ...
    output_dir = Path("/Users/claricepark/Desktop/pro_rice_open_closed_prediction/artifacts/archive/label_coverage")
    output_dir.mkdir(parents=True, exist_ok=True)
    history.to_csv(output_dir / "simulation_history_lightgbm.csv", index=False)
