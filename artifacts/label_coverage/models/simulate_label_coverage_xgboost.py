#!/usr/bin/env python3
"""
Label Coverage Simulation for XGBoost Model

This script runs the iterative active-labeling simulation using XGBoost
...
class LabelCoverageSimulation:
    """Main simulation orchestrator for XGBoost model."""
    ...
    def run_batch(
        ...
        if len(training_data) == 0:
            ...
            # Convert label column to 'open' format (XGBoost expects this)
            ...
            print(f"\n{'='*60}")
            print(f"Training: XGBoost (Policy)")
            print(f"{'='*60}")
            
            model = XGBoostModel(mode="two-stage", feature_bundle="low_plus_medium")
            
            # Calculate class weights for balanced training
            n_samples = len(train_records)
            n_open = (train_records['open'] == 1).sum()
            n_closed = (train_records['open'] == 0).sum()
            
            if n_closed > 0:
                scale_pos_weight = n_closed / n_open  # XGBoost uses this for class balance
            else:
                scale_pos_weight = 1.0
            
            model.fit(train_records, scale_pos_weight=scale_pos_weight)
        ...
        if model is None:
            ...
            p_open_raw = model.predict_proba(batch_unlabeled)
...
def main():
    """Run the XGBoost label coverage simulation."""
    print(f"\n{'='*70}")
    print(f"XGBOOST LABEL-COVERAGE SIMULATION")
    print(f"{'='*70}")
    ...
    history.to_csv(output_dir / "simulation_history_xgboost.csv", index=False)
